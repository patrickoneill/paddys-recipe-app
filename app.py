...
def edit_recipe(task_id):
    ...
    recipeMeal = mongo.db.recipeMeal.find()
    recipeAllergy = mongo.db.recipeAllergy.find()
    return render_template('editRecipe.html', task=recipeCollection, categories=recipeCat, allergy=recipeAllergy, typeOfMeal=recipeMeal)
    

@app.route('/update_recipe/<task_id>', methods=['GET','POST'])
...
app.route('/recipe_score/<task_id>')
def recipe_score(task_id):
    recipeCollection = mongo.db.recipeCollection.find_one({"_id": ObjectId(task_id)})
    score = recipeCollection["recipe_score"]
    score += 1
    mongo.db.recipeCollection.update({'_id': ObjectId(task_id)}, { '$set': { "recipe_score": score }})
    return redirect(url_for('see_recipes', task_id=task_id))
